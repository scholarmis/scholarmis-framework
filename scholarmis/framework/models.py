...
class DataModel(UUIDModel, TimeStampedModel, DirtyFields):
    class Meta:
        abstract = True


class SequenceModel(models.Model):
    seq_number = models.BigIntegerField(blank=True, null=True, db_index=True)
    ...
    def save(self, *args, **kwargs):
        if self.seq_number is None:
            self._generate_sequence_number()
        super().save(*args, **kwargs)


    def _generate_sequence_number(self):
        """
        Generates a thread-safe sequential number using database-level locking
        """
        # Use separate transaction to handle sequence generation
        with transaction.atomic():
            # Get the current maximum sequence number with table lock
            max_seq = self.__class__.objects.select_for_update().aggregate(
                max_seq=Max('seq_number')
            )['max_seq'] or 0
            
            # Handle concurrent creation attempts
            while True:
                next_seq = max_seq + 1
                # Try to atomically claim the sequence number
                updated = self.__class__.objects.filter(seq_number__isnull=True).filter(pk=self.pk) if self.pk else None
                
                # For new instances, just assign the next sequence
                if not self.pk:
                    self.seq_number = next_seq
                    return
                
                # For existing instances, try to claim the number atomically
                if updated and updated.update(seq_number=next_seq):
                    self.seq_number = next_seq
                    return
                
                # If update failed (due to concurrent claim), refresh max
                max_seq = self.__class__.objects.select_for_update().aggregate(
                    max_seq=Max('seq_number')
                )['max_seq'] or 0


class OptionModel(DataModel, SequenceModel):
    SEPARATOR = "_"
